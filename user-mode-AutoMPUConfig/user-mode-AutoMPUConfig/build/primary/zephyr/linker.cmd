 ...
 *(.ARM.exidx* gnu.linkonce.armexidx.*)
 ...
 k_object_assignment_area : { _k_object_assignment_list_start = .; KEEP(*(SORT_BY_NAME(._k_object_assignment.static.*))); _k_object_assignment_list_end = .;; } > FLASH
...
__ramfunc_load_start = LOADADDR(.ramfunc);
        app_smem :
        {
            ...
            "*libc.a:*"(.data .data.* .sdata .sdata.*)
            "*libm.a:*"(.data .data.* .sdata .sdata.*)
            z_data_smem_z_libc_partition_bss_start = .;
            KEEP(*(data_smem_z_libc_partition_bss*))
            "*libc.a:*"(.bss .bss.* .sbss .sbss.* COMMON COMMON.*)
            "*libm.a:*"(.bss .bss.* .sbss .sbss.* COMMON COMMON.*)
            z_data_smem_z_libc_partition_bss_end = .;
            . = ALIGN(_region_min_align); . = ALIGN( 1 << LOG2CEIL(z_data_smem_z_libc_partition_bss_end - z_data_smem_z_libc_partition_part_start));
            z_data_smem_z_libc_partition_part_end = .;
            . = ALIGN(_region_min_align); . = ALIGN( 1 << LOG2CEIL(z_data_smem_partition_shared_bss_end - z_data_smem_partition_shared_part_start));
            z_data_smem_partition_shared_part_start = .;
            KEEP(*(data_smem_partition_shared_data*))
            z_data_smem_partition_shared_bss_start = .;
            KEEP(*(data_smem_partition_shared_bss*))
            z_data_smem_partition_shared_bss_end = .;
            . = ALIGN(_region_min_align); . = ALIGN( 1 << LOG2CEIL(z_data_smem_partition_shared_bss_end - z_data_smem_partition_shared_part_start));
            z_data_smem_partition_shared_part_end = .;
            . = ALIGN(_region_min_align); . = ALIGN( 1 << LOG2CEIL(z_data_smem_partition2_bss_end - z_data_smem_partition2_part_start));
            z_data_smem_partition2_part_start = .;
            KEEP(*(data_smem_partition2_data*))
            z_data_smem_partition2_bss_start = .;
            KEEP(*(data_smem_partition2_bss*))
            z_data_smem_partition2_bss_end = .;
            . = ALIGN(_region_min_align); . = ALIGN( 1 << LOG2CEIL(z_data_smem_partition2_bss_end - z_data_smem_partition2_part_start));
            z_data_smem_partition2_part_end = .;
            . = ALIGN(_region_min_align); . = ALIGN( 1 << LOG2CEIL(z_data_smem_partition1_bss_end - z_data_smem_partition1_part_start));
            z_data_smem_partition1_part_start = .;
            KEEP(*(data_smem_partition1_data*))
            z_data_smem_partition1_bss_start = .;
            KEEP(*(data_smem_partition1_bss*))
            z_data_smem_partition1_bss_end = .;
            . = ALIGN(_region_min_align); . = ALIGN( 1 << LOG2CEIL(z_data_smem_partition1_bss_end - z_data_smem_partition1_part_start));
            z_data_smem_partition1_part_end = .;
            . = ALIGN(_region_min_align);;
            _app_smem_end = .;
        } > RAM AT > FLASH
        ...
        z_data_smem_partition_shared_part_size = z_data_smem_partition_shared_part_end - z_data_smem_partition_shared_part_start;
        z_data_smem_partition_shared_bss_size = z_data_smem_partition_shared_bss_end - z_data_smem_partition_shared_bss_start;
        z_data_smem_partition2_part_size = z_data_smem_partition2_part_end - z_data_smem_partition2_part_start;
        z_data_smem_partition2_bss_size = z_data_smem_partition2_bss_end - z_data_smem_partition2_bss_start;
        z_data_smem_partition1_part_size = z_data_smem_partition1_part_end - z_data_smem_partition1_part_start;
        z_data_smem_partition1_bss_size = z_data_smem_partition1_bss_end - z_data_smem_partition1_bss_start;
 ...
 *(.user_stacks*)
 ...
 priv_stacks_noinit (NOLOAD) : ALIGN_WITH_INPUT
 {
